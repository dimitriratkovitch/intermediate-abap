...
CLASS lcl_flight IMPLEMENTATION.
  ...
  METHOD get_description.
    ...
    txt = replace( val = txt sub = '&date&' with = |{ flight_date
    DATE = USER }| ).
    txt = replace( val  = txt
                   sub  = '&from&'
                   with =
                          connection_details-airport_from_id ).
    txt = replace( val  = txt
                   sub  = '&to&'
                   with =
                          connection_details-airport_to_id ).
...
* currency_code
    ...
    IF flight_raw IS NOT INITIAL.
      ...
      me->planetype = flight_raw-plane_type_id.
      me->seats_max = flight_raw-seats_max.
      me->seats_occ = flight_raw-seats_occupied.
* seats_free = flight_raw-seats_max - flight_raw-seats_occupied.
      me->seats_free = flight_raw-seats_free.
...
CLASS lcl_cargo_flight IMPLEMENTATION.
  ...
  METHOD get_description.
    ...
    r_result = super->get_description( ).
    APPEND |Maximum Load: { maximum_load } { load_unit }| TO
    r_result.
    APPEND |Free Capacity: { get_free_capacity( ) } { load_unit }| TO
    r_result.
  ENDMETHOD.
...
"! Flight Carrier -
"! A factory logic ensures that there is only one instance for the
"!same carrier ID.
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_dr_failed | Instantiation failed - evaluate the exception text for details.
    "!
    "!
...
INTO @DATA(details).
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_dr_failed
        EXPORTING
          textid     = zcx_dr_failed=>carrier_not_found
*          previous   =
          carrier_id = i_carrier_id.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ...
      RAISE EXCEPTION TYPE zcx_dr_failed
        EXPORTING
          textid     = zcx_dr_failed=>carrier_no_read_auth
*          previous   =
          carrier_id = i_carrier_id.
...
*    r_result = REDUCE #(
*    INIT i = 0
*    FOR flight IN me->passenger_flights
*    NEXT i = i + flight->get_free_seats( )
*    ) / lines( me->passenger_flights ).
